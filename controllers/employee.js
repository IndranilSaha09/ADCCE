...
// Function to retrieve employee data based on providers
...
// Handling data retrieval for Provider A

    User.Provider_A.findById(id)
    .exec((err, data) => {
      if (data) 
      {
		  // Rendering employee status page
...
// Similar blocks for Provider B, C, D with respective models
...
// Function to save employee status
const saveemployeestatus =  (offerstatus,req,res) => {
  const id =req.session.passport.user;
  try{
    const id =req.session.passport.user;

 // Handling status updates for Provider A
    User.Provider_A.findById(id)
    .exec((err, data) => {
    if (data) 
    {
...
const addemployee =  (req, res) => {
  ...
  // Retrieving user ID from the session
    ...
    try{
      ...
      if (data) 
      {
		  // Saving new employee data for Provider A
